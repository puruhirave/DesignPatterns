...
PrinterStatePtr PrinterStateMachine::RegisterStateTransitions(PRINTER_STATE printer_state)
{
	...
	switch (printer_state)
	{
		case PRINTER_STATE::OFF : {
			...
			/*transitionMap[PrinterEvent::PowerOn] = { std::bind(&PrinterStateMachine::ProcessPowerOnEvent, this),
													 PRINTER_STATE::OPERATION };*/
...
void PrinterStateMachine::ProcessStateEvent(PrinterEvent event)
{
	...
	//Find event specific state pointer from stateMachineMap
	...
	{
		//Find state transition event from transitionMap, execute and change state
